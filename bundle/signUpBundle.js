...
var signUpScreen = document.querySelectorAll('.form-control');
var signupScreenArr = Array.from(signUpScreen);
...
async function loginRequest(email, password) {
    var config = {
        method: 'post',
        url: _register.url + '/api/auth/login',
        data: {
            user: {
                email: email,
                password: password
...
async function userCredientals() {
    var config = {
        method: 'get',
        url: _register.url + '/api/users/profile',
...
async function signInRequest(username, email, password) {
    var config = {
        method: 'post',
        url: _register.url + '/api/auth/signup',
        data: {
            "user": {
                "userName": username,
                "email": email,
...
exports.signInVariables = signInVariables;
exports.signUpScreen = signUpScreen;
exports.signupScreenArr = signupScreenArr;
exports.signUpVariables = signUpVariables;
...
var registered_User = exports.registered_User = function () {
    function registered_User(email, password) {
        _classCallCheck(this, registered_User);

        this.email = email;
        this.password = password;
    }

    _createClass(registered_User, [{
        key: 'summarize',
        value: function summarize() {
...
document.getElementById("SignIn").addEventListener("click", function () {
    ...
    var registeredUser = new _signUpModel.registered_User(_signUpModel.signInVariables.email.value, _signUpModel.signInVariables.password.value);

    registeredUser.summarize();
    if (registeredUser.email !== "" && registeredUser.password !== "") {
        if (registeredUser.email.indexOf("@") > 0) {
            (0, _signUpModel.loginRequest)(registeredUser.email, registeredUser.password).then(setTimeout(function () {
